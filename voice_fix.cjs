...
function speakerMatchesChar(speaker, charKey) {
    const s = speaker.toLowerCase();
    if (charKey === 'irisReal' || charKey === 'irisVr') return s.includes('iris');
    if (charKey === 'leo') return s === 'leo';
    if (charKey === 'chloe') return s === 'chloe';
    if (charKey === 'maya') return s.includes('maya');
    if (charKey === 'randomStudent') return s.includes('random') || s.includes('student');
    if (charKey === 'crowd') return s.includes('crowd') || s.includes('everyone') || s.includes('class');
    if (charKey === 'mayaChloe') return s.includes('maya') || s.includes('chloe');
    return false;
}

// Build by-scene entries
const byScene = {};
for (const [key, path] of Object.entries(voiceMap)) {
    const ci = key.indexOf(':');
    const sceneId = key.slice(0, ci);
    const origIdx = parseInt(key.slice(ci + 1), 10) - 1;
    const charMatch = path.match(/_([a-zA-Z]+)\.mp3$/);
    const charKey = charMatch ? charMatch[1] : null;
    if (!byScene[sceneId]) byScene[sceneId] = [];
    byScene[sceneId].push({ origIdx, path, charKey });
}

// Resolve correct current index for each voice entry
const correctedMap = {}; // new key -> path
let hasMismatch = false;

for (const [sceneId, entries] of Object.entries(byScene)) {
    const speakers = scenes[sceneId];
    if (!speakers) { console.error(`MISSING SCENE: ${sceneId}`); continue; }
    entries.sort((a, b) => a.origIdx - b.origIdx);

    const charCounts = {};
    for (const e of entries) {
        if (!charCounts[e.charKey]) charCounts[e.charKey] = 0;
        const n = charCounts[e.charKey]++;
        let count = 0, currentIdx = -1;
        for (let i = 0; i < speakers.length; i++) {
            if (speakerMatchesChar(speakers[i], e.charKey)) {
                if (count === n) { currentIdx = i; break; }
                count++;
            }
        }
        const newKey = `${sceneId}:${String(currentIdx + 1).padStart(2, '0')}`;
        correctedMap[newKey] = e.path;
        if (currentIdx !== e.origIdx) {
            hasMismatch = true;
            console.log(`MISMATCH ${sceneId}: ${e.path.replace('/voice/', '')}`);
            console.log(`  was orig:${e.origIdx} -> now currentIdx:${currentIdx} | speaker: ${speakers[currentIdx] || 'NOT FOUND'}`);
        }
    }
}

if (!hasMismatch) console.log('All voice lines match! No mismatches found.');

// Output the final inline voice property for each scene->line
console.log('\n// Inline voice assignments (sceneId -> 0-based lineIdx -> path):');
const bySceneFinal = {};
for (const [key, path] of Object.entries(correctedMap)) {
    const ci = key.indexOf(':');
    const sceneId = key.slice(0, ci);
    const idx = parseInt(key.slice(ci + 1), 10) - 1;
    if (!bySceneFinal[sceneId]) bySceneFinal[sceneId] = {};
    bySceneFinal[sceneId][idx] = path;
}

const outLines = [];
for (const [sceneId, idxMap] of Object.entries(bySceneFinal)) {
    for (const [idx, path] of Object.entries(idxMap)) {
        outLines.push(`  "${sceneId}:${idx}": "${path}",`);
    }
}
outLines.sort().forEach(l => console.log(l));
